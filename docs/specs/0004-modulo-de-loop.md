...
- Matriz refeita com a regra no prompt (2026-09-03), mesma tarefa e mesmos perfis: as tres ferramentas com cota disponivel **perguntaram**, nenhuma apagou. O DeepSeek, que tinha apagado duas vezes com a regra so no bloco, perguntou nas duas rodadas em que a regra veio pelo prompt. Grok continuou em exit 4 por limite do plano free.
- Efeito colateral medido: dizer a restricao no prompt **reduziu** o consumo, porque o agente para de explorar um caminho que seria descartado. Claude caiu de USD 1,01 para 0,69 a preco de tabela, com o cache read caindo de 463k para 394k; Codex caiu de 31.693 para 16.667 tokens, quase metade.
- Revisao item a item do bloco (2026-09-03), aplicando o criterio da DEC-018 a cada regra. Resultado: cinco restricoes precisam do prompt e tres podem ficar so no bloco, porque o agente nao tem como viola-las ou porque o codigo ja as enforca. O unico buraco encontrado foi a ausencia de qualquer instrucao impedindo o agente de editar arquivos de memoria e de regra; em nove rodadas de bancada nenhum agente havia feito isso, entao o risco era teorico, mas o pior caso e silencioso e catastrofico.
- **Variancia medida (2026-09-03).** Mesma tarefa do `guiakit`, mesmos perfis, repetindo a configuracao. Nao e conclusao sobre qualidade de modelo: e sobre quanto o desfecho do loop varia entre rodadas identicas.

| Ferramenta | Rodadas | Desfechos |
|---|---|---|
...
- DEC-019 (buraco achado por pergunta do usuario, 2026-09-03): `CONSENSUS.md` entra na lista de arquivos que o agente do loop nao pode escrever, no bloco e no prompt. A revisao item a item de T-042 listou `SESSION.md`, `MEMORY.md`, `DECISIONS.md`, `AGENTS.md` e specs, e **esqueceu o `CONSENSUS.md`**. E o caso onde o buraco doi mais: uma entrada de consenso declara `Metodo`, `Exposicao previa a outras posicoes` e `Rodada`, e uma rodada de loop tem um agente so. Consenso escrito por um modelo apenas, declarando a propria independencia, e exatamente a fraude que esses campos foram criados para denunciar na 2.2.0. Nenhuma das dez rodadas de bancada escreveu la, entao o risco seguia teorico.
...
- Bancada 3 (2026-09-03), primeira em projeto que **nao e codigo**: manual em Markdown, portao proprio, sete problemas plantados em cinco arquivos. Rodada nas quatro ferramentas mapeadas, com consumo medido.
- Resultado que inverte a leitura ingenua de exit code: a **unica** ferramenta que fechou o portao (DeepSeek) foi a unica que **apagou** a frase com o link quebrado. Claude e Codex sairam com exit 3, perguntando, que era a resposta certa; Grok saiu com exit 4 por cota do plano free, sem produzir nada. Verde nao e sinonimo de bom trabalho, e o loop nao tem como distinguir os dois casos.
- Consumo medido (primeira vez): Claude USD 1,01 a preco de tabela, com 463k de cache read contra 18 tokens de entrada nova, o que confirma que cada tentativa e sessao nova relendo o projeto; Codex 31.693 tokens; Grok 50.058 tokens gastos sem produzir nada; DeepSeek USD 0,0022 de dinheiro real. A escala entre o mais caro e o mais barato passa de 400x em preco de tabela, mas o mais barato foi o que destruiu conteudo, entao comparar so por preco engana.
- Bancada 2 (2026-09-02), para validar o que mudou depois da primeira: subprojeto novo (`durakit`, parser de duracao com doze testes e casos de erro), rodado nas tres ferramentas com **a string exata dos perfis gravados em `MEMORY.md`**, no degrau `executar-dificil`.
- Resultado: as tres fecharam com portao verde na tentativa 1, e as tres acertaram todos os casos que as regras determinam, inclusive os que a suite nao cobre. Diferente da bancada 1, onde duas de tres tinham bug de borda; mudaram duas variaveis ao mesmo tempo (regras escritas como itens numerados com os erros explicitos, e esforco `xhigh` contra o padrao), entao o ganho nao pode ser creditado a uma delas.
- Validado que os perfis executam de verdade, o que a bancada 1 nao cobria: o Codex reportou no proprio cabecalho `model: gpt-5.6-terra` e `reasoning effort: xhigh`, confirmando que `-c model_reasoning_effort="xhigh"` funciona; `--effort xhigh` foi aceito por Claude e Grok.
- Validado `agente=` em rodada real: a evidencia das tres traz a string do perfil inteira, aspas preservadas.
- Validado exit 4 com agente mal configurado de verdade (`codex exec` sem `--skip-git-repo-check` fora de repositorio git): parou na tentativa 1, sem mover tarefa e sem escrever evidencia, em vez de queimar as tres.
- Achado de metodo: um dos casos extras que escrevi (`"  1h30m  "`, com espaco antes e depois) nao era determinado pelas regras, que so falam de espaco **entre os pares**. As tres ferramentas divergiram, duas tolerantes e uma estrita, e as tres leituras se sustentam. O caso foi reclassificado como ambiguo em vez de contado como falha. Serve de ilustracao da propria tese do projeto: onde a especificacao cala, modelos divergem.
