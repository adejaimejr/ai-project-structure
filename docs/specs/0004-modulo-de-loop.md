...
- Variancia observada, registrada para nao virar conclusao apressada: o mesmo `deepseek-v4-flash`, na mesma tarefa e no mesmo degrau, deu tres respostas diferentes ao longo das bancadas: apagou a frase (regra so no bloco), perguntou (duas vezes, regra no prompt) e criou o documento ausente com placeholder honesto `(A preencher.)`, que e o que a regra "Nunca Inferir" ensina. A ultima e n=1 e nao pode ser atribuida a nenhuma mudanca especifica.
