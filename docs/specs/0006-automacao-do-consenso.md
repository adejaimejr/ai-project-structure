...
- **DEC-007 (P-7, forma da entrada; 3 de 3 no que fazer, calibragem escolhida pelo usuario em 2026-09-04 entre Codex e Grok):** a forma da entrada de `CONSENSUS.md` **entra no escopo**: secao repetivel por participante com id arbitrario, nas duas formas (debate e achado), secao propria para agente que falhou, e checks novos valendo **so** para entrada automatizada, por marcador opt-in `**Origem:**`. Os diagnosticos novos sao **AVISO** (posicao do Grok, por simetria com o resto de consenso), nao ERRO como queria o Codex. Fato que barateou a decisao: o validador nunca exigiu heading de posicao nomeado; o congelamento em Codex, Claude e Gemini estava no template. Consequencia aceita: entrada automatizada malformada passa em modo normal e so trava em `--strict`, igual ao resto do arquivo.

- **DEC-008 (P-8, proveniencia; 3 de 3 por entrar, calibragem escolhida pelo usuario em 2026-09-04):** a proveniencia **entra no escopo**, com sidecar guardando bruto e manifesto fora do `CONSENSUS.md` e uma **ponte curta** dentro da entrada (posicao do Grok): o comando integral, diretorio e hashes de insumo e saida ficam **so no manifesto**; a entrada aponta para ele. O Codex queria isso tudo dentro da entrada, lendo a DEC-002 ao pe da letra; perdeu pela rotacao e pela leitura humana. O bruto **nao** pode ficar em `.gitignore`, senao o teste da DEC-001 morre na sessao seguinte. Consequencia aceita: conferir um campo exige abrir dois arquivos.
